"""Tests for src/features/extract_avhubert_embeddings.py."""
...
def test_iter_pair_manifest_rows_filters_splits_and_sorts_by_pair_id(tmp_path):
    from src.features.extract_avhubert_embeddings import iter_pair_manifest_rows
...
def test_unique_video_and_audio_units_dedupes(tmp_path):
    from src.features.extract_avhubert_embeddings import unique_visual_and_audio_units
...
def test_log_failure_writes_canonical_schema(tmp_path):
    from src.features.extract_avhubert_embeddings import log_failure
...
def test_main_skips_existing_and_writes_new(tmp_path, monkeypatch):
    from src.features import extract_avhubert_embeddings as ex
    ...
    with patch("src.features.extract_avhubert_embeddings.AVHubertBackend") as AB, \
         patch("src.features.extract_avhubert_embeddings.extract_mouth_crops",
               return_value=np.zeros((2, 25, 1, 88, 88), dtype=np.float16)), \
         patch("src.features.extract_avhubert_embeddings._load_waveform",
               return_value=np.zeros(16000, dtype=np.float32)):
        AB.from_checkpoint.return_value = fake_adapter
        rc = ex.main([
