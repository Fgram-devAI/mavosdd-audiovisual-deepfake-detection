...
Το \codeid{PROVIDER\_CODEC} map ορίζει τα codec specs των native-MP3 providers:
...
Η λύση υλοποιείται μέσω του set \codeid{CLEAN\_WAV\_PROVIDERS}:
...
Η \codeid{process\_row()} ελέγχει ρητά και τις δύο συνθήκες:
...
Τα clean-WAV spoof providers (π.χ.\ \codeid{openai\_tts}) εξαιρούνται
από το provider weighting:

\begin{english}
\begin{verbatim}
provider_weights = {
    p: w for p, w in raw_provider_counts.items()
    if p in PROVIDER_CODEC
}
\end{verbatim}
\end{english}
...
παρήγαγε τα εξής diagnostics:

\begin{english}
\begin{verbatim}
rows=6367 bonafide=4149 spoof=2218 (clean_wav_spoof=1047)
native-MP3 spoof codec distribution: {elevenlabs: 673, google_tts: 498}
round-trip plan: 2978 × (44100, 128k) + 2218 × (24000, 64k)
total round-tripped = 5196 = 4149 bonafide + 1047 openai_tts
\end{verbatim}
\end{english}
