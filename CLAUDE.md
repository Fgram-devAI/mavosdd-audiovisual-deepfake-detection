...
- **`feat/final-fusion-generated-video` shipped** (2026-07-06): logistic-fusion val ROC-AUC 0.9307 (EER 0.1338, F1 0.8376), MLP val ROC-AUC 0.9175, EER-selected threshold 0.3702. The shipped fusion uses **three** trained heads — `audio_fake_score`, `video_av_fake_score`, `sync_inconsistent_score`. `visual_fake_score` is in the score-CSV schema but blank for all 622 val rows (EfficientNet-B0 sampled-frame baseline is notebook-only in `notebooks/03_visual_frame_baseline_extended_data.ipynb`; extracting it to a stable CLI is deferred to follow-up `feat/visual-frame-score-cli`). Score cache and provenance under `data/derived/final_fusion_scores_{train,val}.csv` + `final_fusion_score_provenance.json` (2904 train / 622 val rows, gitignored). Comparison table at `report/val_eval/final_fusion_comparison.md`. Higgsfield external batch at `report/val_eval/generated_video_batch_scores.csv` — 71/71 scored, 76.06% hit rate at the imported MAVOS-DD val threshold (positive-only stress test — detection rate is a hit rate, not accuracy). Final label semantics: `real → 0`, `echomimic/memo/liveportrait/sonic → 1`; native-audio generated videos stay `1` even when sync-consistency says the audio and lips match. Sync-only ROC-AUC 0.2417 is a **diagnostic / negative control** — SyncNet still correctly detects constructed audio-visual mismatches (see `feat/lip-sync-consistency` per-negative-type metrics), but inconsistency-as-generated-video predictor is worse than chance because MAVOS-DD generated videos come with native, internally-synchronized audio. Do not mistake this for SyncNet failing at its own task.
...
- **Follow-up: `feat/visual-frame-score-cli`.** Extract the EfficientNet-B0 sampled-frame baseline out of `notebooks/03_visual_frame_baseline_extended_data.ipynb` into a stable script (e.g. `src/features/extract_visual_frame_score.py`) + a checkpoint under `models/checkpoints/` + a `--visual-frame-ckpt` flag on `scripts/score_generated_video_batch.py`, then re-run `build_final_fusion_scores` so `visual_fake_score` fills in and enters the fusion input. Frame the resulting head as a **channel-confounded visual-frame baseline** (source folders have disjoint resolution / FPS / codec signatures), not as a semantic visual fake detector.
